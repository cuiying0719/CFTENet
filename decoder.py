...
import math
import os
from tcn_layer import TemporalConvolutionNetwork
...
class TCNDecoder(torch.nn.Module):
    def __init__(self, input_size, output_size, num_channels, kernel_size, dropout):
        ...
        self.bn0 = torch.nn.BatchNorm1d(4)
        self.bn1 = torch.nn.BatchNorm1d(num_channels[-1])
    ...
    def forward(self, embedding, emb_rel, emb_time, triplets, nodes_id=None, mode="train", negative_rate=0, partial_embeding=None):
        ...
        stacked_inputs = self.bn0(stacked_inputs)  # batch_size,2,h_dim
        x = self.inp_drop(stacked_inputs)  # batch_size,2,h_dim
        x = self.tcn(x)  # batch_size,2,h_dim
        x = self.bn1(x)  # batch_size,channels,h_dim
        x = F.relu(x)
        x = self.feature_map_drop(x)
        x = x.view(batch_size, -1)  # batch_size,channels*h_dim
...
class TCNDecoderR(torch.nn.Module):
    def __init__(self, input_size, output_size, num_channels, kernel_size, dropout):
        super(TCNDecoderR, self).__init__()
        self.tcn = TemporalConvolutionNetwork(input_size, num_channels, kernel_size = kernel_size, dropout = dropout)
        self.fc = torch.nn.Linear(num_channels[-1]*200, 200)
        self.inp_drop = torch.nn.Dropout(0.2)
        self.hidden_drop = torch.nn.Dropout(0.2)
