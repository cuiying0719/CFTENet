...
class RGCNLinkDataset(object):
    ...
    def load(self, load_time=True):
        ...
        # --------------
        train_his_path = os.path.join(self.dir, 'train_his.txt')
        valid_his_path = os.path.join(self.dir, 'valid_his.txt')
        test_his_path = os.path.join(self.dir, 'test_his.txt')
        # --------------
        ...
        # -------------------------------
        self.train_his = np.array(_read_triplets_with_his(train_his_path))
        self.valid_his = np.array(_read_triplets_with_his(valid_his_path))
        self.test_his = np.array(_read_triplets_with_his(test_his_path))
        # -------------------------------
...
def _read_triplets_with_his(filename):
    l = []
    for triplet in _read_triplets(filename):  # 读取四元组
        s = int(triplet[0])
        r = int(triplet[1])
        o = int(triplet[2])
        st = int(triplet[3])
        st2 = int(triplet[4])
        l.append([s, r, o, st, st2])
    return l