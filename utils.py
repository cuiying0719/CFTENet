...
def build_sub_graph_with_his(num_nodes, num_rels, triples, use_cuda, gpu):
    def comp_deg_norm(g):
        in_deg = g.in_degrees(range(g.number_of_nodes())).float()
        in_deg[torch.nonzero(in_deg == 0).view(-1)] = 1
        norm = 1.0 / in_deg
        return norm
    src, rel, dst = triples[:, :3].transpose()
    src, dst = np.concatenate((src, dst)), np.concatenate((dst, src))
    rel = np.concatenate((rel, rel + num_rels))
    ...
    g.apply_edges(lambda edges: {'norm': edges.dst['norm'] * edges.src['norm']})
    g.edata['type'] = torch.LongTensor(rel)
    g.edata['tim1'] = torch.LongTensor(np.concatenate((triples[:, 3], triples[:, 3])))
    g.edata['tim2'] = torch.LongTensor(np.concatenate((triples[:, 4], triples[:, 4])))

    # ---------------
    # # 检查第三维和第四维是否相等
    # mask = triples[:, 3] == triples[:, 4]
    # # 应用掩码过滤数组
    # filtered_triples = triples[mask]
...
def split_by_time_with_his(data):
    snapshot_list = []
    snapshot = []
    snapshots_num = 0
    latest_t = 0
...
def load_data(dataset, bfs_level=3, relabel=False):
    if dataset in ['aifb', 'mutag', 'bgs', 'am']:
        ...
        return knwlgrh.load_link(dataset)
                     ...
                     "WIKI"]:
        return knwlgrh.load_from_local(os.path.join(os.getcwd(), "data"), dataset)
    else:
