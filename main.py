...
import random
import utils
from utils import build_sub_graph, build_sub_graph_with_his
...
def test(model, history_list, history_his_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, time_list, history_time_nogt, mode, test_his_list):
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        history_his = build_sub_graph_with_his(num_nodes, num_rels, input_his, use_cuda, args.gpu)
        # ----------------
        # history_glist = [build_sub_graph(num_nodes, num_rels, g, use_cuda, args.gpu) for g in input_list]
        history_glist = [build_sub_graph_with_his(num_nodes, num_rels, g, use_cuda, args.gpu) for g in input_list]
        # ----------------
        ...
        test_triples, final_score, final_r_score = model.predict(history_glist, history_his, num_rels, static_graph, test_triples_input, one_hot_tail_seq, one_hot_rel_seq, use_cuda)
        ...
        if args.multi_step:
            ...
            # input_his = test_his_list[time_idx]
            # -----
            input_list.pop(0)
            input_his = test_his_list[time_idx]
            input_list.append(input_his)
...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    ...
    # -------------
    train_his_list, train_times_his = utils.split_by_time_with_his(data.train_his)
    valid_his_list, valid_times_his = utils.split_by_time_with_his(data.valid_his)
    test_his_list, test_times_his = utils.split_by_time_with_his(data.test_his)
    # -------------
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                if train_sample_num - args.train_history_len<0:
                    input_list = train_his_list[0: train_sample_num]
                    # input_list = train_list[0: train_sample_num]
                else:
                    input_list = train_his_list[train_sample_num - args.train_history_len:train_sample_num]
                    # input_list = train_list[train_sample_num - args.train_history_len: train_sample_num]
                input_his = train_his_list[train_sample_num-1]
                history_his = build_sub_graph_with_his(num_nodes, num_rels, input_his, use_cuda, args.gpu)

                # generate history graph
                # ---------------------------------
                # history_glist = [build_sub_graph(num_nodes, num_rels, snap, use_cuda, args.gpu) for snap in input_list]
                history_glist = [build_sub_graph_with_his(num_nodes, num_rels, snap, use_cuda, args.gpu) for snap in input_list]
                # ---------------------------------
                ...
                loss_e, loss_r, loss_static = model.get_loss(history_glist, history_his, output[0], static_graph, one_hot_tail_seq, one_hot_rel_seq, use_cuda)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Evolution model for TKG forecasting')
    ...
    parser.add_argument("--weight-decay", type=float, default=1e-5, help="weight_decay")
